...
LOGFILE="/var/log/fix-container-103-$(date +%Y%m%d-%H%M%S).log"
touch "$LOGFILE"
chmod 600 "$LOGFILE"
...
cleanup() {
    ...
    if [ $exit_code -ne 0 ]; then
        ...
        pct exec "$CTID" -- systemctl stop waydroid-vnc.service 2>&1 || true
    fi
}
trap cleanup EXIT
...
pct exec "$CTID" -- pkill wayvnc 2>&1 | tee -a "$LOGFILE" || log "No wayvnc to kill"
sleep 2
pct exec "$CTID" -- pkill -9 wayvnc 2>&1 | tee -a "$LOGFILE" || true
pct exec "$CTID" -- pkill sway 2>&1 | tee -a "$LOGFILE" || log "No sway to kill"
sleep 2
pct exec "$CTID" -- pkill -9 sway 2>&1 | tee -a "$LOGFILE" || true
sleep 3
...
# Load environment if it exists in secure location
if [ -f /etc/waydroid/env.sh ]; then
    if [ "$(stat -c %U /etc/waydroid/env.sh)" = "root" ]; then
        source /etc/waydroid/env.sh
    fi
fi
...
nohup su -c "$SWAY_ENV sway" "$DISPLAY_USER" > /var/log/waydroid-sway.log 2>&1 &
sleep 2
...
if [ -z "$SWAY_RUNNING" ]; then
    ...
    cat /var/log/waydroid-sway.log 2>/dev/null || log "No sway log"
    exit 1
fi
...
rm -f "$XDG_RUNTIME_DIR/$WAYLAND_DISPLAY" || true
...
nohup su -c "$WAYVNC_ENV wayvnc 0.0.0.0 5900" "$DISPLAY_USER" > /var/log/waydroid-wayvnc.log 2>&1 &
sleep 3
...
if [ "$WAYVNC_RUNNING" = "false" ]; then
    ...
    cat /var/log/waydroid-wayvnc.log 2>/dev/null || log "No wayvnc log"
    exit 1
fi
...
while true; do
    ...
    if ! pgrep -u "$DISPLAY_USER" sway > /dev/null; then
        ...
        tail -20 /var/log/waydroid-sway.log 2>/dev/null || log "No sway log"
        exit 1
    fi
    ...
    if ! ss -tlnp | grep -q ':5900'; then
        ...
        tail -20 /var/log/waydroid-wayvnc.log 2>/dev/null || log "No wayvnc log"
        exit 1
    fi
